...
Class Master extends DBConnection {
	...
	function save_clinic() {
		...
		if(empty($id)){
			$sql = "INSERT INTO `clinic` set {$data} ";
		}else{
			$sql = "UPDATE `clinic` set {$data} where id = '{$id}' ";
		}
		$check = $this->conn->query("SELECT * FROM `clinic` where `clinicname` = '{$clinicname}' ".($id > 0 ? " and id != '{$id}'" : ""));
		
		if($check->num_rows > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Clinic name already exists.";
		}else{
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = "Clinic has successfully added.";
				else
					$resp['msg'] = "Clinic details has been updated successfully.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_clinic(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `clinic` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
