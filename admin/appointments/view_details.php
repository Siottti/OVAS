<?php 
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT a.*, c.clinicname as clinic,p.breed, m.name as customer, pt.name as pet_type, p.bdate, p.name as pet, m.email, m.contact, m.province, m.city, m.barangay, m.house_street from `appointment_list` a 
            left join pet p on p.id = a.pet_id 
            left join category_list pt on pt.id = p.type 
            left join customer m on m.id = a.customer_id 
            left join clinic c on a.clinic_id = c.id where a.id = '{$_GET['id']}'");
...
$barangay = $barangay;
$address = $province->value . " " . $city->value . " " . $barangay . ", " . $house_street;
?>
<style>
    @media screen {
        .show-print{
                        ...
                        <div class="row">
                            <div class="col-12">
                                <table class="table table-bordered border-info">
                                    ...
                                    <tr>
                                        <th class="text-muted text-white bg-gradient-dark px-2 py-1">Appointment Request ID</th>
                                        <td><?= ($id) ?></td>
                                    </tr>
                                </table>
                            </div>
                                        ...
                                        <table class="table table-stripped table-bordered" data-placeholder='true' id="">
                                            ...
                                            <tbody>
                                                <tr class="border-info">
                                                    ...
                                                    <td class="py-1 px-2 text-right"><?= ucwords($customer) ?></td>
                                                </tr>
                                                <tr class="border-info">
                                ...
                                <div class="col-md-6">
                                    <fieldset>
                                        ...
                                        <table class="table table-stripped table-bordered" data-placeholder='true'>
                                            ...
                                            <tbody>
                                            <tr class="border-info">
                                                    <th class="py-1 px-2 text-light bg-gradient-info">Pet</th>
                                                    <td class="py-1 px-2 text-right"><?= ($pet) ?></td>
                                                </tr>
                                                <tr class="border-info">
                                                ...
                                                <tr class="border-info">
                                                    <th class="py-1 px-2 text-light bg-gradient-info">Birthday</th>
                                                    <td class="py-1 px-2 text-right"><?= ($bdate) ?></td>
                                                </tr>
                                                <tr class="border-info">
...
<script>
    $(function(){
        $('#delete_data').click(function(){
			_conf("Are you sure to delete <b><?= $id ?>\'s</b> from appointment permanently?","delete_appointment",['<?= $id ?>'])
		})
