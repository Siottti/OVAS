	...
	<div class="card-body">
		<div class="container-fluid">
			...
			<form action="" id="manage-user">	
				...
				<?php if(!isset($_SESSION["userdata"]["clinic_id"])) { ?>
				<div class="form-group col-6">
					<label for="clinic_id">Branch</label>
					<select class="form-control" id="clinic_id" name="clinic_id"> 
						<option value="0">Main Branch</option>
						<?php 
							$qry = $conn->query("SELECT * from `clinic`");
							while($row = $qry->fetch_assoc()):						
						?>
							<option value="<?= $row["id"] ?>"  <?php echo isset($meta['clinic_id']) && $meta['clinic_id'] == $row["id"] ? 'selected': '' ?>><?= $row["clinicname"] ?></option>
						<?php endwhile; ?>
					</select>
				</div>
				<?php } else { echo "<input type='hidden' value='".$_SESSION["userdata"]["clinic_id"]."' name='clinic_id'>"; } ?>
				<div class="form-group col-6">
